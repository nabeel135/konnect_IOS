...
class s: UIView {
    ...
    // MARK:- CREATE
    func create(heading:String,login:Bool,view:UIScrollView){
        search.parentbody = view
        search.issignin = login
        let htitle = heading == "" ? "Search":heading
        //HEADER
        search.header.Label(x: 10, y: 0, width: x-50, height: 50, txt: htitle, fontsize: 16, bold: false, cornerRadius: 0, border: 0, borderColor: .clear, alignment: .left, bkcolor: #colorLiteral(red: 1, green: 1, blue: 1, alpha: 1), txtcolor: #colorLiteral(red: 0, green: 0, blue: 0, alpha: 1), view: view)
...
//        searchProductobj[searchProductobj.count-1].title = "title \(searchProductobj.count-1)"
    ...
    func addProduct(grid:Bool,isSignedin:Bool){
        ...
        if grid {
            ...
            for i in 0..<searchProductobj.count {
                ...
                search.pquantity[i].Textfield(x: search.pcheck[i].clickableimg.frame.maxX+5, y: search.pbarbtn[i].clickableimg.frame.maxY+5, width: 30, height: 30, placeholder: "", border: 1, borderRadius: 5, txtAlign: .center, bordercolor: #colorLiteral(red: 0.8039215803, green: 0.8039215803, blue: 0.8039215803, alpha: 1), keyboard: .numberPad, textColor: #colorLiteral(red: 0, green: 0, blue: 0, alpha: 1), bkcolor: #colorLiteral(red: 1.0, green: 1.0, blue: 1.0, alpha: 1.0), view: search.pview[i].view)
                ...
                if isSignedin {
                    ...
                    search.pcheck[i].clickableimg.frame.origin.x = search.pfavbtn[i].clickableimg.frame.minX-25
                    search.pquantity[i].txtfield.frame.origin.x = search.pcheck[i].clickableimg.frame.maxX+5
                    search.pstorebtn[i].clickableimg.frame.origin.x = search.pquantity[i].txtfield.frame.maxX+5
                }else{
                    search.pcheck[i].clickableimg.frame.origin.x = search.pfavbtn[i].clickableimg.frame.minX+10
                    search.pquantity[i].txtfield.frame.origin.x = search.pcheck[i].clickableimg.frame.maxX+5
                    search.pstorebtn[i].clickableimg.frame.origin.x = search.pquantity[i].txtfield.frame.maxX+5
                }
            ...
            for i in 0..<searchProductobj.count {
                ...
                if isSignedin {
                    ...
                    print("else run")
                    search.pcheck[i].clickableimg.frame.origin.x = search.pfavbtn[i].clickableimg.frame.minX-30
                    search.pquantity[i].txtfield.frame.origin.x = search.pcheck[i].clickableimg.frame.maxX+5
                    search.pstorebtn[i].clickableimg.frame.origin.x = search.pquantity[i].txtfield.frame.maxX+5
                }
    ...
    @objc func quoteButton(_ btn:UIButton){
        quotepop.create(quotebtn: btn, inview: search.parentbody)
    }
...
//                productdetail.distributorName = searchProductobj[i].distributorName
                ...
                productdetail.imag = pimag[i].imag.image ?? UIImage(named: "notfound")!
